...
def chain_import_fields_to_fyle(workspace_id, task_settings: TaskSetting, run_in_rabbitmq_worker: bool = False):
    """
    ...
    if custom_properties_tasks is not None:
        chain_tasks.append(
            Task(
                target=task_settings['custom_properties']['func'],
                kwargs={
    ...
    if task_settings['import_categories']:
        chain_tasks.append(
            Task(
                target='fyle_integrations_imports.tasks.trigger_import_via_schedule',
                kwargs={
    ...
    if task_settings['import_items'] is not None and task_settings['import_items']:
        chain_tasks.append(
            Task(
                target='fyle_integrations_imports.tasks.disable_items',
                kwargs={
                    'workspace_id':workspace_id,
    ...
    if task_settings.get('import_dependent_fields'):
        chain_tasks.append(
            Task(
                target=task_settings['import_dependent_fields']['func'],
                kwargs={
    ...
    if run_in_rabbitmq_worker:
        ...
        for task in chain_tasks:
            chain.append(task.target, *task.args, **task.kwargs)
